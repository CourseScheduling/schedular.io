...
router.get('/courses',function *(){
	...
	yield (callback) => {
		...
		_UniCol.find({title:{$in:_CourseArray},term: global.ufv[0].term,year: global.ufv[0].year},{ fields:{_id:0} },(e,d) => {
			if(e)
				return callback(e);
			_this.body = d;
			console.log(e);
...
router.get('/search',function *(){
	...
	yield new Promise((resolve,reject) => {
		_UniCol.find({title: _query,term: global.ufv[0].term,year: global.ufv[0].year},{fields:{title: 1,name: 1,_id: 0},limit:5},(err,docs) => {
			if(err)
